...
class ListItem {
  ...
  startEditItem(target) {
    ...
    createForm.elements.deadline.value = `${date.getFullYear()}-${formatNumber(
      date.getMonth() + 1
  ...
  renderItem() {
    ...
    newSection.appendChild(
      this.createBtn("list-item__mark", "done", this.doneItem)
    );
    newSection.appendChild(
      this.createBtn("list-item__edit", "edit", this.startEditItem)
    );
    newSection.appendChild(
      this.createBtn("list-item__remove", "delete", this.removeItem)
    );
    newSection.appendChild(
...
createForm.onsubmit = function (event) {
  ...
  if (inputs.name.value.length >= 5 && inputs.deadline.value) {
    ...
    if (editIndex >= 0) {
      ...
      let indexToUpdate = listArray.findIndex(
        (item) => +item.listId === editIndex
      );
